...
import { getBooksAccountsReceivableAging } from "../../actions";
...
export const metadata = {
  title: "AR Aging | Slipwise",
};

interface AccountsReceivableAgingPageProps {
  searchParams: Promise<{
    asOfDate?: string;
  }>;
}

export default async function AccountsReceivableAgingPage({
  searchParams,
}: AccountsReceivableAgingPageProps) {
  const params = await searchParams;
  const filters = { asOfDate: params.asOfDate };
  const result = await getBooksAccountsReceivableAging(filters);
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Accounts Receivable Aging</h1>
            <Badge variant={Math.abs(report.variance) <= 0.01 ? "success" : "warning"}>
              {Math.abs(report.variance) <= 0.01 ? "Tied Out" : "Variance"}
            </Badge>
          </div>
          <p className="mt-1 text-sm text-[var(--text-muted)]">
            Invoice-level aging using receivable balances and open document positions.
          </p>
        </div>

        <ExportBooksReportButton
          report="ar-aging"
          filenamePrefix="books-ar-aging"
          filters={filters}
          label="Export CSV"
        />
      </div>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {report.buckets.map((bucket) => (
            <div key={bucket.label} className="rounded-xl border border-[var(--border-soft)] p-4">
              ...
              <p className="mt-1 text-sm text-[var(--text-muted)]">
                {bucket.count} invoice{bucket.count === 1 ? "" : "s"} • {bucket.percentage}%
              </p>
            </div>
          ))}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <h2 className="text-base font-semibold text-[var(--text-primary)]">Open receivables</h2>
        </CardHeader>
        <CardContent className="px-0 py-0">
          <FinanceTable>
            <FinanceTableHeader>
              <FinanceTableHead>Invoice</FinanceTableHead>
              <FinanceTableHead>Customer</FinanceTableHead>
