...
async function InvoiceTable({
  ...
  const { invoices, total, totalPages } = await listInvoices({
    ...
    dateFrom, dateTo, sequenceId, amountMin, amountMax, tagIds,
  });
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3">
          ...
          <div className="flex gap-2">
            {page > 1 && <Link href={`?${buildQuery({ status, search, page: page - 1, dateFrom, dateTo, sequenceId, amountMin, amountMax, tagIds: tagIds?.join(",") })}`} className="rounded px-3 py-1 text-sm text-slate-600 hover:bg-slate-100">Previous</Link>}
            {page < totalPages && <Link href={`?${buildQuery({ status, search, page: page + 1, dateFrom, dateTo, sequenceId, amountMin, amountMax, tagIds: tagIds?.join(",") })}`} className="rounded px-3 py-1 text-sm text-slate-600 hover:bg-slate-100">Next</Link>}
          </div>
        </div>
      )}
    </div>
...
export default async function InvoicesPage({
  ...
  searchParams: Promise<{ status?: string; search?: string; page?: string; view?: string; dateFrom?: string; dateTo?: string; amountMin?: string; amountMax?: string; sequenceId?: string; tagIds?: string; filters?: string }>;
}) {
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="flex items-center rounded-xl bg-white p-1 shadow-sm border border-slate-200">
            <Link
              href={`/app/docs/invoices?${buildQuery({ search: params.search || undefined, tagIds: params.tagIds || undefined })}`}
            ...
            <Link
              href={`/app/docs/invoices?${buildQuery({ view: "sequence", search: params.search || undefined, tagIds: params.tagIds || undefined })}`}
              className={`flex items-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium transition-all ${
                view === "sequence"
                  ? "bg-slate-900 text-white shadow-sm"
                  : "text-slate-500 hover:text-slate-700"
          ...
          <AdvancedFilters
            ...
            extraParams={{ status: status || undefined, search: params.search || undefined, view, tagIds: params.tagIds || undefined }}
            show={params.filters === "open"}
            toggle={buildQuery({ status: status || undefined, search: params.search || undefined, view, tagIds: params.tagIds || undefined, filters: "open" })}
          />
        </div>
        ...
        {view === "list" && (
          <div className="mb-4">
            <StatusFilterChips currentStatus={status} extraParams={{ view: params.view, search: params.search || undefined, tagIds: params.tagIds || undefined }} />
          </div>
        )}
